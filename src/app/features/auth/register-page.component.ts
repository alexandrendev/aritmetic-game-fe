...
export class RegisterPageComponent {
  ...
  submit(): void {
    ...
    this.authService
      ...
      .subscribe({
        ...
        error: (error: unknown) => {
          this.errorMessage.set(this.extractMessage(error, 'Nao foi possivel criar sua conta. Tente novamente.'));
        }
      });
  }
  ...
  private extractMessage(error: unknown, fallback: string): string {
    if (error instanceof HttpErrorResponse) {
      ...
      if (error.status === 400) {
        return 'Cadastro invalido. Confira usuario, email e senha.';
      }
    }
